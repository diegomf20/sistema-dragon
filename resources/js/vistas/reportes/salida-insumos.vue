                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="item in lista">
                                ...
                                <td>{{ item.cantidad_consumo }}</td>
                                <td>{{ item.total_consumo }}</td>
                                <td>{{ (item.total_consumo/item.cantidad_consumo).toFixed(2) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
export default {
    ...
    methods: {
        listar(){
            axios.get(url_base+'/salida-insumos?fecha_inicio='+this.fecha_inicio+'&fecha_fin='+this.fecha_fin)
            .then(response=>{
